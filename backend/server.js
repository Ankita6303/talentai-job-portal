...
app.post('/jobs', requireAdmin, async (req, res) => {
  ...
  if (!title||!department||!description) return res.status(400).json({ error:'Required fields missing' });
  try {
    const r = await pool.query(
    ...
    notifyWhatsAppSubscribers(`🔔 New Job: ${title} at ${department}\n📍 ${location||'Remote'}\nApply at: ${process.env.FRONTEND_URL}`);
